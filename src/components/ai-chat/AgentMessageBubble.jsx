...
const FunctionDisplay = ({ toolCall }) => {
  ...
  if (hideDetails) {
    ...
    const label = isActive
      ...
      : isFailed ? displayProjection.error_label : displayProjection.label;
    return (
      <div className="mt-1.5 text-xs">
        <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg border border-slate-200 bg-white text-slate-600">
          ...
          <StateIcon className={cn("h-3 w-3", isActive && "animate-spin text-indigo-500", isFailed && "text-red-500", !isActive && !isFailed && "text-green-600")} />
        </div>
      </div>
    );
  }
...
export default function AgentMessageBubble({ message }) {
  ...
  return (
    <div className={cn("flex gap-3", isUser ? "justify-end" : "justify-start")}>
      {!isUser && (
        <div className="h-8 w-8 rounded-xl bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center mt-0.5 shrink-0">
          <Zap className="h-4 w-4 text-white" />
        </div>
      )}
      <div className={cn("max-w-[85%]", isUser && "flex flex-col items-end")}>
        {message.content && (
          <div className={cn("rounded-2xl px-4 py-2.5", isUser ? "bg-indigo-600 text-white" : "bg-white border border-slate-200 shadow-sm")}>
            {isUser ? (
              ...
              <ReactMarkdown
                ...
                components={{
                  ...
                  th: ({ children }) => <th className="px-3 py-1.5 bg-slate-50 text-left font-semibold border-b border-slate-200">{children}</th>,
