// Demo profile datasets. Each profile has compact but representative data
// across Locations, Equipment, Technicians, Compliance, Budgets, etc.
//
// Cross-entity references use `_location_key` and `_equipment_key` strings
// that the loader resolves to real IDs after each parent is created.
...
export const DEMO_PROFILES = {
  snowy_hydro: {
    ...
    data: {
      ...
      Alert: [
        { equipment_id: 'placeholder', title: 'Tumut 1 U2 bearing temp rising', severity: 'warning', type: 'threshold_exceeded', status: 'active', message: 'Bearing temperature trending +3°C over baseline', sensor_type: 'temperature', recommended_action: 'Schedule vibration analysis' },
        { equipment_id: 'placeholder', title: 'Talbingo piezometer drift', severity: 'warning', type: 'degradation_trend', status: 'active', message: 'Piezometer P-12 reading drift detected' },
      ],
      SavingsLedgerEntry: [
